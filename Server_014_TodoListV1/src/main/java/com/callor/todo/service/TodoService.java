 ...
 * 
 * Map<String,Class>형식으로 type을 정의하고 
 * Map.put<"이름",객체>형식으로 데이터를 추가하기
 * 
 * Class class = map.get("이름") 형식으로
 * 	값을 읽어서 다른 변수에 담을 수 있다
 * System.out.println(map.get("이름")) 형식으로
 * 		Console에 출력하기 
 * 
 * VO(DTO)를 선언하지 않고 임시로 VOtype의 데이터를 사용하는 방법
 * 
 * Map을 사용할 때 key값을 String 형으로 지정하면
 * 값을 put(),get() 할 때 key값을 ""로 감싼 문자열 
 * 형식으로 사용해야 하기 때문에 
 * 많은 오타로 인한 논리적오류가 발생 할 수 있다.
 * 이러한 문제를 방지하기 위해 key값을 별도 static 변수로 만들어두고
 * 활용하는 방법을 사용한다
 * 
 *  
 */
