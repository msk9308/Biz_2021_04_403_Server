...
table.td_list td.doit {
	...
	/*
	width : 무조건 폭을 지정
	max-width : 포함된 text(content) 길이에 따라 
	길이에 조절을 최소 크기만 제한 
	*/
	max-width:80%;
	/*
...
.underline {
	text-decoration: 3px uderline red y;
}
...
// 이 후의 다른 Event가 정상적으로 설정 될 수 있도록 선언
	...
	<table class="td_list" >
		<c:forEach items="${TDLIST}" var="TD">
			<tr data-seq="${TD.td_seq}" data-edate="${TD.td_edate}">
				<td class="sdate ${empty TD.td_edate ? '' : 'uderline' }">${TD.td_sdate}<br />${TD.td_stime}</td>
