...
public interface BookService {
	...
	//도서정보 데이터를 1개 전달 받아서 DB insert를 수행
	...
	//데이터가 없을때는 null이 있을때는 실제 데이터가 담긴 DTO가 return된다
	public BookDTO findById(String bk_isbn);
	//도서명을 1개 전달받아서 도서명으로 조회를하고 결과를 return하는 기능 수행
	public List<BookDTO> findByTitle(String bk_title);
	//변경할 도서 데이터 1개를 전달받아서 데이터를 갱신(변경,수정)하는 기능 수행 
	public void update(BookVO bookVO);
	//삭제할 도서의 ISBN을 1개 전달받아서 한개의 도서정보를 삭제하는 기능 수행
